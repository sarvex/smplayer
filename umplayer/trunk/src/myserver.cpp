...
Connection::Connection(QTcpSocket * s) 
{
	...
	sendText(QString("UMPlayer %1").arg(umplayerVersion()));
...
void Connection::parseLine(QString str) {
	...
	QRegExp rx_loadsub("^load_sub (.*)");
        QRegExp rx_opendir("^open_dir (.*)");
        QRegExp rx_adddir("^add_dir (.*)");


	if (str.toLower() == "hello") {
		sendText(QString("Hello, this is UMPlayer %1").arg(umplayerVersion()));
	}
	else
	if (str.toLower() == "help") {
	...
	if (rx_loadsub.indexIn(str) > -1) {
        ...
        if (rx_opendir.indexIn(str) > -1) {
                QString dir = rx_opendir.cap(1);
                qDebug("Connection::parseLine: asked to open directory '%s'", dir.toUtf8().data());
                sendText("OK, open directory sent to GUI");
                emit receivedOpenDirectory(dir);
        }
        else
        if (rx_adddir.indexIn(str) > -1) {
                QString dir = rx_adddir.cap(1);
                qDebug("Connection::parseLine: asked to add directory '%s'", dir.toUtf8().data());
                sendText("OK, add directory sent to GUI");
                emit receivedAddDirectory(dir);
        }
	else
...
void MyServer::newConnection_slot() {
	...
	connect(c, SIGNAL(receivedLoadSubtitle(QString)),
        ...
        connect(c, SIGNAL(receivedOpenDirectory(QString)),
            this, SIGNAL(receivedOpenDirectory(QString)));
        connect(c, SIGNAL(receivedAddDirectory(QString)),
                this, SIGNAL(receivedAddDirectory(QString)));
}

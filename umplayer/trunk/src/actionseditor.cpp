...
int ActionsEditor::findActionAccel(const QString & accel, int ignoreRow) {
        ...
        {
            int row = -1;
            QStringList accels = QString(accel).remove(' ').split(',');
            foreach(QString ac, accels)
            {
                row = findActionAccel(ac, ignoreRow);
                if(row != -1) break;
            }
            return row;
        }
	...
	for (int row=0; row < actionsTable->rowCount(); row++) {
		QTableWidgetItem * i = actionsTable->item(row, COL_SHORTCUT);	
#if USE_MULTIPLE_SHORTCUTS                
                QStringList toCheck = i->text().remove(' ').split(',');
                if ( (i) &&  toCheck.contains(accel) ) {

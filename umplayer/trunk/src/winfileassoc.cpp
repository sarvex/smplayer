/*  umplayer, GUI front-end for mplayer.
	...
	System-wide associations require writing to HKEY_CLASSES_ROOT and we don't want to get our hands dirty with that.
        Each user on the computer can configure his own set of file associations for UMPlayer, which is extremely cool.
...
WinFileAssoc::WinFileAssoc(const QString ClassIdVideo, const QString ClassIdAudio, const QString ClassIdList, const QString AppName)
{
        m_ClassIdVideo = ClassIdVideo;
        m_ClassIdAudio = ClassIdAudio;
        m_ClassIdList = ClassIdList;
	m_AppName = AppName; 
...
{        
	...
	//Check if classId exists in the registry
        if (!RegCR.contains(m_ClassIdVideo) && !RegCU.contains("Software/Classes/" + m_ClassIdVideo))
	{
		//If doesn't exist (user didn't run the setup program), try to create the ClassId for current user.
                if (!CreateClassId(QApplication::applicationFilePath(), "UMPlayer Media Player"))
			return 0; 
	}
	...
	foreach(const QString& fileExtension, fileExtensions)
	{
		QString ExtKeyName = QString("Software/Microsoft/Windows/CurrentVersion/Explorer/FileExts/.%1").arg(fileExtension);
                QString ClassesKeyName;                
                if(ext.video().contains(fileExtension))
                    ClassesKeyName = m_ClassIdVideo;
                else if(ext.audio().contains(fileExtension))
                    ClassesKeyName = m_ClassIdAudio;
                else if(ext.playlist().contains(fileExtension))
                    ClassesKeyName = m_ClassIdList;
		...
		QString KeyVal = RegCU.value(CUKeyName + "/.").toString(); 
                ...
                if (KeyVal.length() == 0 || KeyVal == ClassesKeyName)
		{
		...
		KeyVal = RegCU.value(ExtKeyName + "/Application").toString(); 
                if (KeyVal != ClassesKeyName || KeyVal != m_ClassId2)
                {
		...
		//Create the associations                
                if (QSysInfo::WindowsVersion >= QSysInfo::WV_NT && !QSysInfo::WindowsVersion >= QSysInfo::WV_VISTA)
		{
                        RegCU.setValue(CUKeyName + "/.", ClassesKeyName); 		//Extension class
                        //RegCU.setValue(ExtKeyName + "/Progid", ClassesKeyName);	//Explorer FileExt association
                        RegCU.remove(ExtKeyName + "/Progid");	//deassoc fix
                }
		else
		{
			//Windows ME/98/95 support
                        RegCR.setValue("." + fileExtension + "/.", ClassesKeyName);
		}
        ...
        {
                count = VistaSetAppsAsDefault(fileExtensions);
        }
        else if(count)
        {
	...
	foreach(const QString& fileExtension, extensionsToCheck)
	{
		...
		if (QSysInfo::WindowsVersion >= QSysInfo::WV_NT)
		{
			QString FileExtsKey = QString("Software/Microsoft/Windows/CurrentVersion/Explorer/FileExts/.%1").arg(fileExtension);
                        ...
                        if(QSysInfo::WindowsVersion >= QSysInfo::WV_VISTA)
                        {
                            CurClassId = RegCU.value(FileExtsKey + "/UserChoice/Progid").toString();
                        }
                        else
                        {
                            CurClassId = RegCU.value(FileExtsKey + "/Progid").toString();
                        }
			QString CurAppId = RegCU.value(FileExtsKey + "/Application").toString();

			if (CurClassId.size())	//Registered with Open With... / ProgId ?
			{
                                bRegistered = (CurClassId == ClassesKeyName) || (0 == CurClassId.compare(m_ClassId2, Qt::CaseInsensitive));
			}
			else
			if (CurAppId.size())
			{
				//If user uses Open With..., explorer creates it's own ClassId under Application, usually "umplayer.exe"
                                bRegistered = (CurAppId == ClassesKeyName) || (0 == CurAppId.compare(m_ClassId2, Qt::CaseInsensitive));
			}
                        else
                        {
				//No classId means that no associations exists in Default Programs or Explorer
				//Check the default per-user association 
                                QString CurUserClassId = RegCU.value("Software/Classes/." + fileExtension + "/.").toString();
                                if(CurUserClassId.size())
                                {
                                    bRegistered = (CurUserClassId == ClassesKeyName);
                                }
                                else
                                {
                                    bRegistered = RegCR.value("." + fileExtension + "/.").toString() == ClassesKeyName;
                                }
...
int WinFileAssoc::RestoreFileAssociations(const QStringList& fileExtensions)
{
	...
	foreach(const QString& fileExtension, fileExtensions)
	{
                ...
                if(ext.video().contains(fileExtension))
                    ...
                    ClassesKeyName = m_ClassIdList;
                QString ExtKeyName;
                ExtKeyName = QString("Software/Microsoft/Windows/CurrentVersion/Explorer/FileExts/.%1").arg(fileExtension);
                QString ExtKeyName2 = QString("Software/Microsoft/Windows/CurrentVersion/Explorer/FileExts/.%1").arg(fileExtension);

                QString OldApp = RegCU.value(ExtKeyName2 + "/MPlayer_Backup_Application").toString();
                QString OldClassId = RegCU.value("Software/Classes/." + fileExtension + "/MPlayer_Backup").toString();

		//Restore old explorer ProgId
                /*if (!OldProgId.isEmpty() && OldProgId != ClassesKeyName)
			RegCU.setValue(ExtKeyName + "/Progid", OldProgId);
		else
                {*/
                QString CurProgId = RegCU.value(ExtKeyName + "/Progid").toString();
                if ((CurProgId == ClassesKeyName) || (0 == CurProgId.compare(m_ClassId2, Qt::CaseInsensitive))) //Only remove if we own it
                        RegCU.remove(ExtKeyName + "/Progid");
                //}

		//Restore old explorer Application 
                if (!OldApp.isEmpty() && OldApp != ClassesKeyName)
                        RegCU.setValue(ExtKeyName2 + "/Application", OldApp);
		else
		{
                        QString CurApp = RegCU.value(ExtKeyName2 + "/Application").toString();
                        if ((CurApp == ClassesKeyName) || (0 == CurApp.compare(m_ClassId2, Qt::CaseInsensitive))) //Only remove if we own it
                                RegCU.remove(ExtKeyName2 + "/Application");
		}

		if (QSysInfo::WindowsVersion >= QSysInfo::WV_NT)
		{
			//Restore old association for current user
                        if (!OldClassId.isEmpty() && OldClassId != ClassesKeyName)
				RegCU.setValue("Software/Classes/." + fileExtension + "/.", OldClassId); 
			else
			{
                                if (RegCU.value("Software/Classes/." + fileExtension + "/.").toString() == ClassesKeyName) //Only remove if we own it
		...
		else
		{
			...
			{
                                if (RegCR.value("." + fileExtension + "/.").toString() == ClassesKeyName)
        ...
        SHChangeNotify(SHCNE_ASSOCCHANGED, SHCNF_IDLIST, 0, 0);
        if(QSysInfo::WindowsVersion >= QSysInfo::WV_VISTA  && fileExtensions.count() > 0)
        {
            QStringList  extensions = fileExtensions;
            QString arg = extensions.replaceInStrings(QRegExp("^(.*)$"), ".\\1").join(",");
            QProcess proc;
            proc.start("\"" + QApplication::applicationDirPath() + "/Restore.exe\" " + arg);
            bool ok = proc.waitForFinished();
            if(ok)
                qDebug() << "Restore Process Finished.";
            else
                qDebug() << "Restore Process had a problem. " << proc.errorString();
...
bool WinFileAssoc::CreateClassId(const QString& executablePath, const QString& friendlyName)
{
	...
	if (QSysInfo::WindowsVersion >= QSysInfo::WV_NT) 
	{
                classIdV = "Software/Classes/" + m_ClassIdVideo;
                classIdA = "Software/Classes/" + m_ClassIdAudio;
                classIdL = "Software/Classes/" + m_ClassIdList;
		RootKeyName = "HKEY_CURRENT_USER";
	}
	else 
	{
                classIdV = m_ClassIdVideo;
                classIdA = m_ClassIdAudio;
                classIdL = m_ClassIdList;
	...
	//Add our ProgId to the HKCR classes
        Reg.setValue(classIdV + "/shell/open/FriendlyAppName", friendlyName);
        Reg.setValue(classIdA + "/shell/open/FriendlyAppName", friendlyName);
        Reg.setValue(classIdL + "/shell/open/FriendlyAppName", friendlyName);


        Reg.setValue(classIdV + "/shell/open/command/.", QString("\"%1\" \"%2\"").arg(appPath, "%1"));
        Reg.setValue(classIdA + "/shell/open/command/.", QString("\"%1\" \"%2\"").arg(appPath, "%1"));
        Reg.setValue(classIdL + "/shell/open/command/.", QString("\"%1\" \"%2\"").arg(appPath, "%1"));



        Reg.setValue(classIdV + "/DefaultIcon/.", QString("\"%1\",2").arg(appPath));
        Reg.setValue(classIdA + "/DefaultIcon/.", QString("\"%1\",1").arg(appPath));
        Reg.setValue(classIdL + "/DefaultIcon/.", QString("\"%1\",1").arg(appPath));


	//Add "Enqueue" command
        Reg.setValue(classIdV + "/shell/enqueue/.", QObject::tr("Enqueue in UMPlayer"));
        Reg.setValue(classIdA + "/shell/enqueue/.", QObject::tr("Enqueue in UMPlayer"));
        Reg.setValue(classIdL + "/shell/enqueue/.", QObject::tr("Enqueue in UMPlayer"));


        Reg.setValue(classIdV + "/shell/enqueue/command/.", QString("\"%1\" -add-to-playlist \"%2\"").arg(appPath, "%1"));
        Reg.setValue(classIdA + "/shell/enqueue/command/.", QString("\"%1\" -add-to-playlist \"%2\"").arg(appPath, "%1"));
        Reg.setValue(classIdL + "/shell/enqueue/command/.", QString("\"%1\" -add-to-playlist \"%2\"").arg(appPath, "%1"));
	return true; 
}
...
bool WinFileAssoc::RemoveClassId()
{
	...
	if (!RegCU.isWritable() || RegCU.status() != QSettings::NoError)
        ...
        RegCU.remove(classIdV);
        RegCU.remove(classIdA);
        RegCU.remove(classIdL);
	return true; 
}

//Windows Vista specific implementation
//Add libole32.a library if compiling with mingw.
//In umplayer.pro, under win32{ :
//	LIBS += libole32
#ifdef WIN32
#include <windows.h>

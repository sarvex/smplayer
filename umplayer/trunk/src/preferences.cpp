...
void Preferences::reset() {
	...
	if (QFile::exists(Paths::configPath() + "/screenshots")) {
        ...
        recordings_directory = Paths::configPath() + "/recordings";
#endif
	...
	fast_chapter_change = false;
        ...
        cache_for_streams_non_seekable = 500;
	...
	default_size = QSize(580, 440);
        ...
        hide_video_window_on_audio_files = false;
#endif
...
#ifdef Q_OS_WIN
	// Some people reported umplayer doesn't start with this option enabled
    ...
    /* ****************
       ...
       **************** */
        ...
        floating_control_margin = 20;
        floating_control_width = 80; //100 %
        floating_control_animated = false;
    ...
    /*********
      Auto-update
      **********/
        lastUpdateTime = QDateTime::fromTime_t(0);
        uid = QUuid();
        ...
        /************************
          ...
          *********************/
        checkDefault = true;
        exceptExtensions = QStringList();

        fontCacheBuilt = false;
        playCount = 0;
}
...
void Preferences::save() {
	...
	set->setValue("screenshot_directory", screenshot_directory);
        ...
        set->setValue("recordings_directory", recordings_directory);
	...
	set->setValue("file_settings_method", file_settings_method);
        set->setValue("font_cache", fontCacheBuilt);
	...
	set->setValue("cache_for_files", cache_for_files);
        set->setValue("cache_for_streams_seekable", cache_for_streams_seekable);
        set->setValue("cache_for_streams_non_seekable", cache_for_streams_non_seekable);
	...
	set->setValue("cache_for_tv", cache_for_tv);
        ...
        set->setValue("recording_quality", recording_quality );
        set->setValue("recording_format", recording_format);
	...
	set->setValue("freetype_support", freetype_support);
        ...
        set->setValue("language_set", language_is_set );
	...
	set->setValue("log_umplayer", log_umplayer);
    ...
    set->setValue( "auto_get_info", auto_get_info );
    ...
    /* ********
       ...
       ******** */
        ...
        set->setValue("date", lastUpdateTime.toTime_t());
        set->setValue("guid", uid.toString());
        set->setValue("os", os);
        set->endGroup();
        ...
        set->beginGroup("recording");
        set->setValue("history", downloadHistory);
        set->endGroup();
        ...
        set->beginGroup("FileTypes");
        set->setValue("check_default", checkDefault);
        set->setValue("except_extensions", exceptExtensions);
        set->endGroup();
...
void Preferences::load() {
	...
	screenshot_directory = set->value("screenshot_directory", screenshot_directory).toString();
        ...
        recordings_directory= set->value("recordings_directory", recordings_directory).toString();
	...
	file_settings_method = set->value("file_settings_method", file_settings_method).toString();
        fontCacheBuilt = set->value("font_cache", false).toBool();
        playCount = set->value("playCount", 0).toInt();
	...
	cache_for_files = set->value("cache_for_files", cache_for_files).toInt();
        cache_for_streams_seekable = set->value("cache_for_streams_seekable", cache_for_streams_seekable).toInt();
        cache_for_streams_non_seekable = set->value("cache_for_streams_non_seekable", cache_for_streams_non_seekable).toInt();
	...
	cache_for_tv = set->value("cache_for_tv", cache_for_tv).toInt();
        ...
        recording_quality = set->value("recording_quality", recording_quality).toInt();
        recording_format = set->value("recording_format", recording_format).toInt();
	...
	freetype_support = set->value("freetype_support", freetype_support).toBool();
        ...
        language_is_set = set->value("language_set", language_is_set).toBool();
	...
	log_umplayer = set->value("log_umplayer", log_umplayer).toBool();
	...
	resize_method = set->value("resize_method", resize_method).toInt();
        ...
        skin = set->value("skin", skin).toString();
	...
	add_to_playlist_consecutive_files = set->value("add_to_playlist_consecutive_files", add_to_playlist_consecutive_files).toBool();
        ...
        auto_get_info = set->value( "auto_get_info", auto_get_info ).toBool();
    ...
    /* *******
       ...
       ******* */
        set->beginGroup("update");
        lastUpdateTime = QDateTime::fromTime_t(set->value("date", lastUpdateTime.toTime_t()).toInt());
        uid = QUuid(set->value("guid", uid.toString()).toString());
        if(uid.isNull()) uid = QUuid::createUuid();
        set->endGroup();

        /***************************
          Recording Dialog History
          ************************/

        set->beginGroup("recording");
        downloadHistory = set->value("history").toList();
        set->endGroup();
        ...
        set->beginGroup("FileTypes");
        checkDefault = set->value("check_default", checkDefault).toBool();
        exceptExtensions = set->value("except_extensions").toStringList();
        set->endGroup();
}

...
static QRegExp rx_fontcache_scan("^Scanning .*");
...
void MplayerProcess::parseLine(QByteArray ba) {
    ...
    if (rx_av.indexIn(line) > -1) {
                QString  secStr = rx_av.cap(0);
                double sec = rx_av.cap(1).toDouble();
                if( secStr.startsWith("A:") && rx_av.indexIn(line, secStr.length()) > -1){
                    if(rx_av.cap(0).startsWith("V:")){
                        sec = rx_av.cap(1).toDouble();
                    }
                }
		...
		// (or if sent, umplayer will ignore anyway...)
		...
		if (rx_clip_software.indexIn(line) > -1) {
			...
			md.clip_software = s;
                ...
                if (rx_fontcache_start.indexIn(line) > -1) {
			//qDebug("MplayerProcess::parseLine: updating font cache");
                        emit receivedUpdatingFontCacheStart();
		}
                else

                if (rx_fontcache_scan.indexIn(line.trimmed()) > -1) {
                        QString text = rx_fontcache_scan.capturedTexts().value(0);
                        //qDebug("MplayerProcess::parseLine: Scanning file ");
                        emit receivedScanningFont(text);
                }
                else

                if (rx_fontcache_end.indexIn(line) > -1) {
                        //qDebug("MplayerProcess::parseLine: updating font cache");
                        emit receivedUpdatingFontCacheEnd();
                }
		else

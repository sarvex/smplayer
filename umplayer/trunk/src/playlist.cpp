	...
	: QWidget(parent,f) 
{
	...
	connect( core, SIGNAL(mediaLoaded()), this, SLOT(getMediaInfo()) );
        ...
        border->setBGColor(toolbar->palette().color(toolbar->backgroundRole()));
...
void Playlist::createTable() {
	...
	listView->setSelectionMode(QAbstractItemView::ExtendedSelection);
        ...
        pal.setColor(QPalette::AlternateBase, QColor("#f0f6ff"));
	...
	*/
        listView->setIconSize( QPixmap(":/Playlist/ok.png").size() );
	...
	connect( listView, SIGNAL(cellActivated(int,int)),
        ...
        connect(listView, SIGNAL(removeSelected()), this, SLOT(removeSelected()));
}
...
void Playlist::retranslateStrings() {
	listView->setHorizontalHeaderLabels( QStringList() << "   " <<
        ...
        openAct->change( Images::icon("playlist_open"), tr("&Load") );
        saveAct->change( Images::icon("playlist_save"), tr("&Save") );
	...
	if (qApp->isLeftToRight()) {
        ...
        moveUpAct->change( Images::icon("playlist_up"), tr("Move &up") );
        moveDownAct->change( Images::icon("playlist_down"), tr("Move &down") );
	...
	// Icon
        ...
        setWindowTitle( tr( "UMPlayer - Playlist" ) );
}
...
void Playlist::updateView() {
	...
	for (int n=0; n < pl.count(); n++) {
		...
		if (pl[n].played()) {
                    listView->setIcon(n, COL_PLAY, QPixmap(":/Playlist/ok.png") );
		} else {
			listView->setIcon(n, COL_PLAY, QPixmap() );
                }
                listView->setRowHeight(n, 17);
...
void Playlist::setCurrentItem(int current) {
	QIcon play_icon;
	if (qApp->isLeftToRight()) {
                play_icon = QPixmap(":/Playlist/playing.png");
	} else {
            QPixmap icon = QPixmap(":/Playlist/playing.png");
            play_icon = Images::flip(&icon);
	}
...
bool Playlist::save_m3u(QString file) {
    ...
    if ( f.open( QIODevice::WriteOnly ) ) {
		...
		stream << "#EXTM3U" << "\n";
                stream << "# Playlist created by UMPlayer " << umplayerVersion() << " \n";
...
void Playlist::addDirectory(QString dir) {
	addOneDirectory(dir);
        ...
        if (pref->recursive_add_directory) {
...
void Playlist::saveSettings() {
	...
	set->endGroup();
        ...
        if (pref->save_playlist_in_config) {
		//Save current list
...
void Playlist::toggleRepeat(bool checked)
{
    repeatAct->setChecked(checked);
}

void Playlist::toggleShuffle(bool checked)
{
    shuffleAct->setChecked(checked);
}
...
void Playlist::checkNextPrevAvailability()
{
    int current = listView->currentRow();
    if (current > -1) {
        if(current > 0)
        {
            emit prevAvailable(true);
        }
        else
        {
            emit prevAvailable(false);
        }
        if(current == listView->rowCount() - 1)
        {

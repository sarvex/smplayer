...
void PrefPlaylist::setData(Preferences * pref) {
	...
	setAddConsecutiveFiles( pref->add_to_playlist_consecutive_files );
        setDirectoryRecursion(pref->recursive_add_directory);
        setSavePlaylistOnExit(pref->save_playlist_in_config);
        setPlayFilesFromStart(pref->play_files_from_start);
        setAutoGetInfo(pref->auto_get_info);
}
...
void PrefPlaylist::getData(Preferences * pref) {
	...
	pref->add_to_playlist_consecutive_files = addConsecutiveFiles();
        pref->recursive_add_directory = directoryRecursion();
        pref->save_playlist_in_config = savePlaylistOnExit();
        pref->play_files_from_start = playFilesFromStart();
        pref->auto_get_info = autoGetInfo();
}
...
void PrefPlaylist::createHelp() {
	...
	setWhatsThis(auto_add_to_playlist_check, tr("Automatically add files to playlist"),
                tr("If this option is enabled, every time a file is opened, UMPlayer "
	...
	setWhatsThis(add_consecutive_files_check, tr("Add consecutive files"),
                tr("If this option is enabled, UMPlayer will look for consecutive "

	...
	: QObject( parent ) 
{
	...
	connect( proc, SIGNAL(error(QProcess::ProcessError)),
        ...
        connect( this, SIGNAL(logLineAvailable(QString)),
                 mplayerwindow, SLOT(testForScanningFinished(QString)));
        connect(proc, SIGNAL(receivedScanningFont(QString)),
                mplayerwindow, SLOT(appendScanningMessage(QString)));
	...
	connect( proc, SIGNAL(receivedScreenshot(QString)),
        ...
        connect( proc, SIGNAL(receivedUpdatingFontCacheStart()),
             this, SLOT(displayUpdatingFontCacheStart()) );
        connect( proc, SIGNAL(receivedUpdatingFontCacheStart()),
             this, SIGNAL(fontCachingStarted()) );
        connect( proc, SIGNAL(receivedUpdatingFontCacheEnd()),
             this, SLOT(displayUpdatingFontCacheEnd()) );
        connect( proc, SIGNAL(receivedUpdatingFontCacheEnd()),
             this, SIGNAL(fontCachingFinished()) );
        connect(proc, SIGNAL(receivedScanningFont(QString)), this, SLOT(displayFontScanning(QString)));
	...
	connect( this, SIGNAL(aboutToStartPlaying()),
        ...
        connect(mplayerwindow, SIGNAL(fontBeingScanned(QString)),
                this, SIGNAL(showPermanentMessage(QString)));
        connect(mplayerwindow, SIGNAL(fontBeingScanned(QString)),
                this, SIGNAL(buffering()));
	...
	DiscName::test();
        ...
        proc->setBufferName(QString("umplayer-%1").arg(pid));
        mplayerwindow->videoLayer()->setSharedMemory(proc->bufferName());
#endif
...
void Core::openStream(QString name, QString title) {
	...
	*/
	...
	if (proc->isRunning()) {
        ...
        if(mdat.type == TYPE_STREAM )
            start_sec = (int) qMin(mset.current_sec, mdat.duration);
        else
            start_sec = mset.current_sec;
...
void Core::startMplayer( QString file, double seek ) {
...
#endif
#ifdef Q_OS_MACX
                proc->addArgument(QString("corevideo:buffer_name=%1").arg(proc->bufferName()));
	...
	qDebug("Core::startMplayer: priority of umplayer process set to %d", app_p);
	#endif
	...
	if (mset.current_video_id != MediaSettings::NoneSelected) {
                // Disabled because of a bug in mplayer matroska demuxer
...
#endif
		                      break;
                case TYPE_YOUTUBE       : cache = pref->cache_for_streams_seekable;break;
                case TYPE_STREAM 	: cache = pref->cache_for_streams_non_seekable;break;
...
void Core::displayUpdatingFontCacheStart() {
        qDebug("Core::displayUpdatingFontCacheStart");
        emit showMessage( tr("Updating the font cache. This may take some seconds...") );
}

void Core::displayUpdatingFontCacheEnd() {
        qDebug("Core::displayUpdatingFontCacheEnd");
        emit showMessage( tr("Caching font completed.") );
}

void Core::displayFontScanning(QString line)
{
        qDebug("Core::displayFontScanning");
        //emit showMessage(tr(line.toUtf8().constData()));
}

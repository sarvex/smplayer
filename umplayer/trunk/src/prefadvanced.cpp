...
void PrefAdvanced::setData(Preferences * pref) {
	...
	setLogSmplayer( pref->log_umplayer );
...
void PrefAdvanced::setLogSmplayer(bool b) {
	log_umplayer_check->setChecked(b);
}

bool PrefAdvanced::logSmplayer() {
	return log_umplayer_check->isChecked();
}
...
void PrefAdvanced::createHelp() {
	...
	setWhatsThis(mplayer_use_window_check, tr("Run MPlayer in its own window"),
        tr("If you check this option, the MPlayer video window won't be "
           "embedded in UMPlayer's main window but instead it will use its "
	...
	setWhatsThis(shortnames_check, tr("Pass short filenames (8+3) to MPlayer"),
		tr("Currently MPlayer can't open filenames which contains characters "
           ...
           "UMPlayer to pass to MPlayer the short version of the filenames, "
	...
	setWhatsThis(log_umplayer_check, tr("Log UMPlayer output"),
		tr("If this option is checked, UMPlayer will store the debugging "
           "messages that UMPlayer outputs "
           "(you can see the log in <b>Options -> View logs -> UMPlayer</b>). "
           "This information can be very useful for the developer in case "
           "you find a bug." ) );

	setWhatsThis(log_mplayer_check, tr("Log MPlayer output"),
		tr("If checked, UMPlayer will store the output of MPlayer "
	...
	setWhatsThis(log_filter_edit, tr("Filter for UMPlayer logs"),
		tr("This option allows to filter the UMPlayer messages that will "

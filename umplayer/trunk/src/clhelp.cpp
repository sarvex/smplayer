...
QString CLHelp::help(bool html) {
	...
	s += formatHelp( "-config-path", QObject::tr(
		"specifies the directory where umplayer will store its configuration "
        "files (umplayer.ini, umplayer_files.ini...)"), html );
	...
	s += formatHelp( QObject::tr("media"), QObject::tr(
                "'media' is any kind of file that UMPlayer can open. It can "
        ...
        "handle the playlist, not UMPlayer."), html );

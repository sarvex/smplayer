...
public:
	...
	//! Saves the line from the umplayer output
        void recordUmplayerLog(QString line);
...
public slots:
	...
	virtual void helpCLOptions();
        virtual void helpHomepage();
	virtual void helpTips();
	virtual void helpAbout();
	virtual void helpAboutQt();

        virtual void shareUmplayer();
	...
	virtual void toggleFrameCounter(bool);
        ...
        void facebookAlert();
        void pauseIfNeeded(Core::State state);


protected slots:
        virtual void closeWindow(bool quit = true);
	...
	virtual void showPopupMenu( QPoint p );
        virtual void changeSkin(QAction* action);
	/*
	...
	*/
	...
	virtual void remoteLoadSubtitle(QString file);
        virtual void remoteOpenDirectory(QString file);
        virtual void remoteAddDirectory(QString file);
	...
	void autosaveMplayerLog();
        ...
        void updateNowResult(QNetworkReply* reply);
        void showFontCacheMesage();
        void hideFontCacheMesage();
...
protected:
	...
	void configureDiscDevices();
        ...
        void checkUpdateNow();
        void askForUpdate(QString action, QString url);
        void changeUpdateDate();
        void updateNow(QString url, bool binary);
...
protected:
	...
	MyAction * showLogSmplayerAct;
        MyAction * showRecordingsAct;

	// Menu Help
	MyAction * showFAQAct;
	MyAction * showCLOptionsAct; // Command line options
        MyAction * homepageAct;
...
#endif
        // UMPlayer action addition
        MyAction * youtubeAct;
        MyAction * shoutcastAct;
        MyAction * recordAct;
        MyAction * equalizerAct;
        YoutubeSearchBox* youtubeBox;
	...
	QString pending_actions_to_run;
        int floatingWidgetHeight;
        QPointer<FaderWidget> fade;
...
private:
	...
	QString mplayer_log;
	QString umplayer_log;

	bool ignore_show_hide_events;
        YTDialog* youTubeDialog;
        SCDialog* shoutcastDialog;
        QTimer* resizeFrameStep;        
};

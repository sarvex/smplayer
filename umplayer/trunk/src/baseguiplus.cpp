	...
	: BaseGui( parent, flags )
{
	...
	mainwindow_pos = pos();
        ...
        tray = new QSystemTrayIcon( Images::icon(logos), this );

        tray->setToolTip( "UMPlayer" );
	...
	connect( showTrayAct, SIGNAL(toggled(bool)),
        ...
        showTrayAct->setChecked(true);

        optionsMenu->insertAction( sepAct1 ,showTrayAct);
...
void BaseGuiPlus::closeWindow(bool quit) {
        qDebug("BaseGuiPlus::closeWindow");
        /*if (tray->isVisible() && quit) {
		//e->ignore();
		exitFullscreen();
		...
		if (pref->balloon_count > 0) {
                        tray->showMessage( "UMPlayer",
                                tr("UMPlayer is still running here"),
        	    QSystemTrayIcon::Information, 3000 );
			pref->balloon_count--;
		}

        } else*/ {
                BaseGui::closeWindow(quit);
	}
	//tray->hide();

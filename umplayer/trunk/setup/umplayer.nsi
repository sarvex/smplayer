;Installer script for win32/win64 UMPlayer
...
!ifdef VER_REVISION
  !define UMPLAYER_VERSION "${VER_MAJOR}.${VER_MINOR}.${VER_BUILD}.${VER_REVISION}"
  !define UMPLAYER_PRODUCT_VERSION "${VER_MAJOR}.${VER_MINOR}.${VER_BUILD}.${VER_REVISION}"
!else ifndef VER_REVISION
  !define UMPLAYER_VERSION "${VER_MAJOR}.${VER_MINOR}.${VER_BUILD}"
  !define UMPLAYER_PRODUCT_VERSION "${VER_MAJOR}.${VER_MINOR}.${VER_BUILD}.0"
!endif

!ifdef WIN64
  !define UMPLAYER_BUILD_DIR "umplayer-build64"
!else
  !define UMPLAYER_BUILD_DIR "umplayer-build"
!endif

  !define UMPLAYER_REG_KEY "Software\UMPlayer"
  !define UMPLAYER_APP_PATHS_KEY "Software\Microsoft\Windows\CurrentVersion\App Paths\umplayer.exe"
  !define UMPLAYER_DEF_PROGS_KEY "Software\Clients\Media\UMPlayer"

  !define UMPLAYER_UNINST_EXE "uninst.exe"
  !define UMPLAYER_UNINST_KEY "Software\Microsoft\Windows\CurrentVersion\Uninstall\UMPlayer"
...
;General
  ...
  ;Name and file
  Name "UMPlayer ${UMPLAYER_VERSION}"
  BrandingText "UMPlayer for Windows v${UMPLAYER_VERSION}"
!ifdef WIN64
  OutFile "output\umplayer-${UMPLAYER_VERSION}-x86_64.exe"
!else
  OutFile "output\umplayer-${UMPLAYER_VERSION}-x86.exe"
!endif

  ;Version tab properties
  VIProductVersion "${UMPLAYER_PRODUCT_VERSION}"
  VIAddVersionKey "ProductName" "UMPlayer"
  VIAddVersionKey "ProductVersion" "${UMPLAYER_VERSION}"
  VIAddVersionKey "FileVersion" "${UMPLAYER_VERSION}"
  VIAddVersionKey "LegalCopyright" ""
!ifdef WIN64
  VIAddVersionKey "FileDescription" "UMPlayer Installer (64-bit)"
!else
  VIAddVersionKey "FileDescription" "UMPlayer Installer (32-bit)"
!endif

  ;Default installation folder
!ifdef WIN64
  InstallDir "$PROGRAMFILES64\UMPlayer"
!else
  InstallDir "$PROGRAMFILES\UMPlayer"
!endif

  ;Get installation folder from registry if available
  InstallDirRegKey HKLM "${UMPLAYER_REG_KEY}" "Path"
...
;Variables
  ...
  Var UMPlayer_Path
  Var UMPlayer_UnStrPath
  Var UMPlayer_StartMenuFolder

;--------------------------------
;Interface Settings

  ;Installer/Uninstaller icons
  !define MUI_ICON "umplayer-orange-installer.ico"
  !define MUI_UNICON "umplayer-orange-uninstaller.ico"

  ;Misc
  !define MUI_WELCOMEFINISHPAGE_BITMAP "umplayer-orange-wizard.bmp"
  !define MUI_UNWELCOMEFINISHPAGE_BITMAP "umplayer-orange-wizard-un.bmp"
  ...
  !define MUI_FINISHPAGE_RUN $INSTDIR\umplayer.exe
  ...
  !define MUI_LANGDLL_REGISTRY_KEY "${UMPLAYER_UNINST_KEY}"
  !define MUI_LANGDLL_REGISTRY_VALUENAME "NSIS:Language"

  ;Memento Settings
  !define MEMENTO_REGISTRY_ROOT HKLM
  !define MEMENTO_REGISTRY_KEY "${UMPLAYER_REG_KEY}"

  ;Start Menu Settings
  !define MUI_STARTMENUPAGE_DEFAULTFOLDER "UMPlayer"
  !define MUI_STARTMENUPAGE_NODISABLE
  !define MUI_STARTMENUPAGE_REGISTRY_ROOT HKLM
  !define MUI_STARTMENUPAGE_REGISTRY_KEY "${UMPLAYER_UNINST_KEY}"
...
;Pages
  ...
  !insertmacro MUI_PAGE_LICENSE "${UMPLAYER_BUILD_DIR}\Copying.txt"
  ...
  !insertmacro MUI_PAGE_STARTMENU "SMP_SMenu" $UMPlayer_StartMenuFolder
...
;--------------------------------
;Main UMPlayer files
Section $(Section_UMPlayer) SecUMPlayer
  ...
  ${If} $Reinstall_Uninstall == 1
    ...
    ${If} $Reinstall_UninstallButton_State == 1
      Exec '"$UMPlayer_UnStrPath" /X'
      Quit
    ${ElseIf} $Reinstall_OverwriteButton_State == 1
      ${If} "$INSTDIR" == "$UMPlayer_Path"
        ExecWait '"$UMPlayer_UnStrPath" /S /R _?=$UMPlayer_Path'
      ${Else}
        ExecWait '"$UMPlayer_UnStrPath" /S /R'
      ${EndIf}
    ${EndIf}
  ...
  SetOutPath "$INSTDIR"
  File "${UMPLAYER_BUILD_DIR}\*"

  ;UMPlayer docs
  SetOutPath "$INSTDIR\docs"
  File /r "${UMPLAYER_BUILD_DIR}\docs\*.*"

  ;Qt imageformats
  SetOutPath "$INSTDIR\imageformats"
  File /r "${UMPLAYER_BUILD_DIR}\imageformats\*.*"

  ;UMPlayer key shortcuts
  SetOutPath "$INSTDIR\shortcuts"
  File /r "${UMPLAYER_BUILD_DIR}\shortcuts\*.*"

  SetOutPath "$PLUGINSDIR"
  File 7za.exe

  ;Initialize to 0 if don't exist (based on error flag)
  ReadRegDWORD $R0 HKLM "${UMPLAYER_REG_KEY}" Installed_MPlayer
  ${If} ${Errors}
    WriteRegDWORD HKLM "${UMPLAYER_REG_KEY}" Installed_MPlayer 0x0
  ${EndIf}

  ReadRegDWORD $R0 HKLM "${UMPLAYER_REG_KEY}" Installed_Codecs
  ${If} ${Errors}
    WriteRegDWORD HKLM "${UMPLAYER_REG_KEY}" Installed_Codecs 0x0
  ${EndIf}
...
SectionGroup $(ShortcutGroupTitle)
  ...
  ${MementoSection} $(Section_DesktopShortcut) SecDesktopShortcut
    ...
    SetOutPath "$INSTDIR"
    CreateShortCut "$DESKTOP\UMPlayer.lnk" "$INSTDIR\umplayer.exe"
  ...
  ${MementoSection} $(Section_StartMenu) SecStartMenuShortcut
    ...
    !insertmacro MUI_STARTMENU_WRITE_BEGIN SMP_SMenu
      CreateDirectory "$SMPROGRAMS\$UMPlayer_StartMenuFolder"
      CreateShortCut "$SMPROGRAMS\$UMPlayer_StartMenuFolder\UMPlayer.lnk" "$INSTDIR\umplayer.exe"
      WriteINIStr    "$SMPROGRAMS\$UMPlayer_StartMenuFolder\UMPlayer on the Web.url" "InternetShortcut" "URL" "http://smplayer.sourceforge.net/umplayer.php"
      CreateShortCut "$SMPROGRAMS\$UMPlayer_StartMenuFolder\Uninstall UMPlayer.lnk" "$INSTDIR\${UMPLAYER_UNINST_EXE}"
...
SectionGroup $(MPlayerGroupTitle)
  ...
  Section $(Section_MPlayer) SecMPlayer
    ...
    File /r "${UMPLAYER_BUILD_DIR}\mplayer\*.*"

    WriteRegDWORD HKLM "${UMPLAYER_REG_KEY}" Installed_MPlayer 0x1
  ...
  Section /o $(Section_MPlayerCodecs) SecCodecs
    ...
    ${If} ${FileExists} "$PLUGINSDIR\version-info"
      ReadINIStr $Codec_Version "$PLUGINSDIR\version-info" umplayer mplayercodecs
    ${Else}
    ...
    IfFileExists "$INSTDIR\mplayer\codecs\*.dll" codecsInstSuccess codecsInstFailed
      codecsInstSuccess:
        WriteRegDWORD HKLM "${UMPLAYER_REG_KEY}" Installed_Codecs 0x1
        Goto done
      codecsInstFailed:
...
${MementoSection} $(Section_IconThemes) SecThemes
  ...
  File /r "${UMPLAYER_BUILD_DIR}\themes\*.*"
...
${MementoSection} $(Section_Translations) SecTranslations
  ...
  File /r "${UMPLAYER_BUILD_DIR}\translations\*.*"
...
Section -Post
  ...
  ;Uninstall file
  WriteUninstaller "$INSTDIR\${UMPLAYER_UNINST_EXE}"

  ;Store installed path & version
  WriteRegStr HKLM "${UMPLAYER_REG_KEY}" "Path" "$INSTDIR"
  WriteRegStr HKLM "${UMPLAYER_REG_KEY}" "Version" "${UMPLAYER_VERSION}"

  ;Allows user to use 'start umplayer.exe'
  WriteRegStr HKLM "${UMPLAYER_APP_PATHS_KEY}" "" "$INSTDIR\umplayer.exe"
...
!ifdef WIN64
  WriteRegStr HKLM "${UMPLAYER_UNINST_KEY}" "DisplayName" "$(^Name) (x64)"
!else
  WriteRegStr HKLM "${UMPLAYER_UNINST_KEY}" "DisplayName" "$(^Name)"
!endif
  WriteRegStr HKLM "${UMPLAYER_UNINST_KEY}" "DisplayIcon" "$INSTDIR\umplayer.exe"
  WriteRegStr HKLM "${UMPLAYER_UNINST_KEY}" "DisplayVersion" "${UMPLAYER_VERSION}"
  WriteRegStr HKLM "${UMPLAYER_UNINST_KEY}" "HelpLink" "http://smplayer.sourceforge.net/forum"
  WriteRegStr HKLM "${UMPLAYER_UNINST_KEY}" "Publisher" "Ricardo Villalba"
  WriteRegStr HKLM "${UMPLAYER_UNINST_KEY}" "UninstallString" "$INSTDIR\${UMPLAYER_UNINST_EXE}"
  WriteRegStr HKLM "${UMPLAYER_UNINST_KEY}" "URLInfoAbout" "http://smplayer.sourceforge.net"
  WriteRegStr HKLM "${UMPLAYER_UNINST_KEY}" "URLUpdateInfo" "http://smplayer.sourceforge.net"
  WriteRegDWORD HKLM "${UMPLAYER_UNINST_KEY}" "NoModify" "1"
  WriteRegDWORD HKLM "${UMPLAYER_UNINST_KEY}" "NoRepair" "1"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
  !insertmacro MUI_DESCRIPTION_TEXT ${SecUMPlayer} $(Section_UMPlayer_Desc)
...
!macro WriteRegStrSupportedTypes EXT
  WriteRegStr HKLM  "${UMPLAYER_DEF_PROGS_KEY}\Capabilities\FileAssociations" ${EXT} "MPlayerFileVideo"
!macroend

!macro MacroRemoveUMPlayer
  ...
  Delete "$DESKTOP\UMPlayer.lnk"
  Delete "$SMPROGRAMS\$UMPlayer_StartMenuFolder\UMPlayer.lnk"
  Delete "$SMPROGRAMS\$UMPlayer_StartMenuFolder\UMPlayer on the Web.url"
  Delete "$SMPROGRAMS\$UMPlayer_StartMenuFolder\Uninstall UMPlayer.lnk"
  RMDir "$SMPROGRAMS\$UMPlayer_StartMenuFolder"
  ...
  DeleteRegKey HKLM "${UMPLAYER_REG_KEY}"
  DeleteRegKey HKLM "${UMPLAYER_APP_PATHS_KEY}"
  DeleteRegKey HKLM "${UMPLAYER_DEF_PROGS_KEY}"
  DeleteRegKey HKLM "${UMPLAYER_UNINST_KEY}"
  DeleteRegKey HKCR "MPlayerFileVideo"
  DeleteRegValue HKLM "Software\RegisteredApplications" "UMPlayer"
...
Function ${UN}RunCheck
  ...
  retry_runcheck:
  FindProcDLL::FindProc "umplayer.exe"
  IntCmp $R0 1 0 +3
    MessageBox MB_RETRYCANCEL|MB_ICONEXCLAMATION $(UMPlayer_Is_Running) /SD IDCANCEL IDRETRY retry_runcheck
    Abort
...
!ifdef WIN64
  ...
  SetRegView 32
  ClearErrors
  ReadRegStr $R0 HKLM "${UMPLAYER_UNINST_KEY}" "UninstallString"
...
!endif
  ...
  System::Call 'kernel32::CreateMutexW(i 0, i 0, t "UMPlayerSetup") i .r1 ?e'
  Pop $R0
...
Function .onInstFailed
  ...
  !insertmacro MacroRemoveUMPlayer

  Delete "$INSTDIR\${UMPLAYER_UNINST_EXE}"
  RMDir "$INSTDIR"
...
Function CheckPreviousVersion
  ...
  ClearErrors
  ReadRegStr $Previous_Version HKLM "${UMPLAYER_REG_KEY}" "Version"
  ReadRegStr $UMPlayer_UnStrPath HKLM "${UMPLAYER_UNINST_KEY}" "UninstallString"
  ReadRegStr $UMPlayer_Path HKLM "${UMPLAYER_REG_KEY}" "Path"
  ...
  ${VersionCompare} $Previous_Version ${UMPLAYER_VERSION} $Previous_Version_State
...
Function LoadPreviousSettings
  ...
  !insertmacro MUI_STARTMENU_GETFOLDER "SMP_SMenu" $UMPlayer_StartMenuFolder
...
Function PageReinstall
  ...
  ${NSD_CreateText} 10u 15u 290u 14u "$UMPlayer_Path"
  Pop $R0
...
Function RegisterDefaultPrograms
  ...
  WriteRegStr HKCR "MPlayerFileVideo\DefaultIcon" "" '"$INSTDIR\umplayer.exe",1'
  WriteRegStr HKCR "MPlayerFileVideo\shell\enqueue" "" "Enqueue in UMPlayer"
  WriteRegStr HKCR "MPlayerFileVideo\shell\enqueue\command" "" '"$INSTDIR\umplayer.exe" -add-to-playlist "%1"'
  WriteRegStr HKCR "MPlayerFileVideo\shell\open" "FriendlyAppName" "UMPlayer Media Player"
  WriteRegStr HKCR "MPlayerFileVideo\shell\open\command" "" '"$INSTDIR\umplayer.exe" "%1"'

  ;Modify the list of extensions added in the MacroAllExtensions macro
  WriteRegStr HKLM "${UMPLAYER_DEF_PROGS_KEY}" "" "UMPlayer"
  WriteRegStr HKLM "${UMPLAYER_DEF_PROGS_KEY}\Capabilities" "ApplicationDescription" $(Application_Description)
  WriteRegStr HKLM "${UMPLAYER_DEF_PROGS_KEY}\Capabilities" "ApplicationName" "UMPlayer"
  WriteRegStr HKLM "Software\RegisteredApplications" "UMPlayer" "${UMPLAYER_DEF_PROGS_KEY}\Capabilities"
...
Section Uninstall
  ...
  ;Make sure UMPlayer is installed from where the uninstaller is being executed.
  IfFileExists $INSTDIR\umplayer.exe umplayer_installed
    MessageBox MB_YESNO $(Uninstaller_NotInstalled) /SD IDNO IDYES umplayer_installed
  ...
  IfErrors 0 +2
  ExecWait '"$INSTDIR\umplayer.exe" -uninstall'

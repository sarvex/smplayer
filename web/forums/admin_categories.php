...
$result = $db->query('SELECT id, disp_position, visible FROM '.$db->prefix.'categories ORDER BY disp_position') or error('Unable to fetch category list', __FILE__, __LINE__, $db->error());
...
$cat_visible[$i] = ($cat_visible[$i] == 'on')? '1': '0';

list($cat_id, $position) = $db->fetch_row($result);

$db->query('UPDATE '.$db->prefix.'categories SET cat_name=\''.$db->escape($cat_name[$i]).'\', disp_position='.$cat_order[$i].', visible=\''.$cat_visible[$i].'\' WHERE id='.$cat_id) or error('Unable to update category', __FILE__, __LINE__, $db->error());
	}
...
$result = $db->query('SELECT id, cat_name, disp_position, visible FROM '.$db->prefix.'categories ORDER BY disp_position') or error('Unable to fetch category list', __FILE__, __LINE__, $db->error());
$num_cats = $db->num_rows($result);
...
<?php
	...
	for ($i = 0; $i < $num_cats; ++$i)
	{
list(, list($cat_id, $cat_name, $position, $visible)) = @each($cat_list);

?>
							<tr>
								<td><input type="text" name="cat_name[<?php echo $i ?>]" value="<?php echo pun_htmlspecialchars($cat_name) ?>" size="35" maxlength="80" /></td>
								<td><input type="text" name="cat_order[<?php echo $i ?>]" value="<?php echo $position ?>" size="3" maxlength="3" /></td>
								<td><input type="checkbox" name="cat_visible[<?php echo $i ?>]"<?php if($visible == "1") echo' checked="checked"';?> /></td>
								<td>&nbsp;</td>
							</tr>
<?php

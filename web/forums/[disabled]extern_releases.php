...
require PUN_ROOT.'forums/include/dblayer/common_db.php';
...
//
if ($action == 'active' || $action == 'new')
{
	$order_by = ($action == 'active') ? 't.last_post' : 't.posted';
	$forum_sql = '';

	// Was any specific forum ID's supplied?
	if (isset($fid) && $fid != '')
	{
		$fids = explode(',', trim($fid));
	...
	if (isset($type) && strtoupper($type) == 'RSS')
	{
		$rss_description = ($action == 'active') ? $lang_common['RSS Desc Active'] : $lang_common['RSS Desc New'];
		$url_action = ($action == 'active') ? '&amp;action=new' : '';
	...
	{
		$show = isset($show) ? intval($show) : 15;
		if ($show < 1 || $show > 50)
			$show = 15;

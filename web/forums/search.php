...
<?php
			...
			else
			{
				...
				if ($search_set[$i]['question'] == "")
					...
					$subject = $lang_polls['Poll'] . ': <a href="viewtopic.php?id='.$search_set[$i]['tid'].'">'.pun_htmlspecialchars($search_set[$i]['subject']).'</a> <span class="byuser">'.$lang_common['by'].'&nbsp;'.pun_htmlspecialchars($search_set[$i]['poster']).'</span><br /> [ '.pun_htmlspecialchars($search_set[$i]['question']).' ]';

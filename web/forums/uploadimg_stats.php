...
//============= STATS CODE START =============
	    ...
	    $result7 = $db->query('SELECT id, username, num_posts FROM '.$db->prefix.'users WHERE id = '.$kode);
        $krydit = $db->fetch_assoc($result7);
        $hierisdit = $krydit ['username'];
        $hierisposts = $krydit['num_posts'];
        $result9 = $db->query('SELECT u.id, u.username, u.title, u.num_posts, u.registered, g.g_id, g.g_user_title FROM '.$db->prefix.'users AS u LEFT JOIN '.$db->prefix.'groups AS g ON g.g_id=u.group_id WHERE u.id = '.$kode);
...
?>
	    ...
	    <?php 
            ...
            $sumuploads = $sumuploads + $nuwearray[$pos]["count"];
	    ...
	    ?>
	           <tr>
					<td class="tcr"><?php echo "<a href='uploadimg_view.php?view=gallery&id=".$nuwearray[$pos][id]."'>".$nuwearray[$pos][name]; ?></td>
				    <td class="tcr"><?php echo $nuwearray[$pos]["title"]; ?></td>
                    <td class="tc3"><?php echo $nuwearray[$pos]["count"]; ?></td>

...
class Text_Figlet
{
     ...
     * @access public
     * @return mixed error or true for success
     */
    function loadFont($filename, $loadgerman = true)
    {
        ...
        if (substr($filename, -3, 3) == '.gz') {
            ...
            if (!function_exists('gzcompress')) {
                return 'Cannot load gzip compressed fonts since gzcompress() is not available.';
            }
        } else {
            $compressed = false;
        }
        ...
        if (!($fp = fopen($filename, 'rb'))) {
            return 'Cannot open figlet font file ' . $filename;
        }
...
//            Max_Length      Old_Layout
        ...
        if (substr($header[0], 0, 5) <> 'flf2a') {
            return 'Unknown FIGlet font format.';
        }
